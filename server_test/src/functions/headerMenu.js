...
export function getLabelLinkGuia() {
    var menuLabel = [
        ...
        },
        {
            ...
            link: '/buenas_practicas/intro',
            pos: 0
        },
        {
            label: 'Próximos pasos',
            link: '/guia_de_gestion/intro',
            pos: 0
        },
        {
...
export function getPositionMenuSelected(numberPage, width, height) {
...
export function menuCircles(svg, width, height, selectedPosition, opacity, noSelected) {
    ...
    var x_tooltipPosition = [
        rp(520, 'x', width, height),
        rp(625, 'x', width, height),
        rp(705, 'x', width, height),
        rp(700, 'x', width, height),
        rp(855, 'x', width, height),
        rp(980, 'x', width, height),
        rp(1040, 'x', width, height),
        rp(1210, 'x', width, height),
        rp(1205, 'x', width, height),
        rp(1420, 'x', width, height),
        rp(1430, 'x', width, height),
        rp(1540, 'x', width, height),
    ]
    var w_tooltipPosition = [
        rp(130, 'x', width, height),
        rp(140, 'x', width, height),
        rp(170, 'x', width, height),
        rp(90, 'x', width, height),
        rp(80, 'x', width, height),
        rp(190, 'x', width, height),
        rp(300, 'x', width, height),
        rp(330, 'x', width, height),
        rp(500, 'x', width, height),
        rp(150, 'x', width, height),
        rp(150, 'x', width, height),
        rp(150, 'x', width, height),
    ]
    ...
    for (var i = 0; i < 12; i++) {
        ...
        setHtmlText(svg,
            ...
            'white',
            '')
            //console.log('x_tooltipPosition['+i+'] ' + x_tooltipPosition[i]);
        svg.append("circle")
            ...
            .on("click", function () {
                ...
                d3.select('#rectWhiteFade')
                    ...
                    .duration(50)
                    .attr('opacity', 1);
                window.location.href = menuLabel[index].link
            })
            .on('mouseover', function () {
                var index = d3.select(this).attr('indexElement');
                //console.log('x_tooltipPosition['+index+'] ' + x_tooltipPosition[index]);
                svg.select('#tooltip' + index).style('opacity', 1)
                svg.select('#menuLabel' + index).style('opacity', 1)
            })
            .on('mouseout', function () {
                var index = d3.select(this).attr('indexElement');
                svg.select('#tooltip' + index).style('opacity', 0)
                svg.select('#menuLabel' + index).style('opacity', 0)
            })
    }
    if (noSelected == 0) {
        svg.append("circle")
...
export function breadCrumbGuia(svg, positionPage, width, height) {
